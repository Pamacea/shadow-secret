...
"#;
    ...
    fn test_create_project_config() {
        ...
        assert!(content.contains("source: \".enc.env\""));
        assert!(content.contains("engine: \"sops\""));
        assert!(content.contains(&format!("age_key_path: \"{}\"", age_key_path.display())));

        // Verify targets section exists
        assert!(content.contains("targets:"));
        assert!(content.contains("name:"));
        assert!(content.contains("path:"));
        assert!(content.contains("placeholders:"));

        // Verify it has usage instructions
