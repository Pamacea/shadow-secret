...
"#,
    ...
    // Step 4: Create global.enc.env (empty, unencrypted initially)
...
# This file will be encrypted with SOPS after adding secrets
#
# To add secrets:
#   1. Decrypt temporarily: sops --decrypt global.enc.env > global.env.tmp
#   2. Add your secrets to global.env.tmp
#   3. Encrypt again: sops --encrypt global.env.tmp > global.enc.env
#   4. Remove temporary file: rm global.env.tmp

# Example secrets (replace with actual values):
# GLOBAL_API_KEY=your_api_key_here
# GLOBAL_DATABASE_URL=your_database_url_here
"#;

        fs::write(&global_enc_env, enc_env_content)
            .with_context(|| format!("Failed to write global.enc.env to: {:?}", global_enc_env))?;
        println!("   ✓ Created: {:?}", global_enc_env);
    }
    println!();
...
# - Create a shadow-secret.yaml in your project directory
# - Set vault.source to absolute or relative path to global.enc.env
