...
"#;
    ...
    // Get the path to the age key for documentation
    let key_path_display = if let Ok(key_path) = std::env::var("SOPS_AGE_KEY_FILE") {
        key_path
    } else {
...
# For SOPS to encrypt/decrypt, you need:
#
# 1. Age key location: {}
#
...
#
# 3. To edit secrets (opens in your $EDITOR):
...
"#,
        key_path_display, key_path_display
    );
